...
struct PizziniCryptoCoreTests {
    ...
    func sessionSerializeContinuesRatchet() throws {
        ...
        // Live send on the sending chain before the snapshot.
        let m1 = try alice.encryptSealed(
            peer: bobId, messageId: Data(repeating: 0x01, count: 16),
            plaintext: Data("hi".utf8)
        )
        _ = try bob.decryptSealed(m1)

        // Snapshot Alice and rehydrate.
        let snapshot = try alice.serialize()
        let alice2 = try Session(serialized: snapshot)

        // Continue on the SAME sending chain (Bob hasn't replied) — must NOT be
        // a duplicate, proving the sending-chain counter survived the rehydrate.
        let m3 = try alice2.encryptSealed(
            peer: bobId, messageId: Data(repeating: 0x02, count: 16),
            plaintext: Data("still here".utf8)
        )
        let received = try bob.decryptSealed(m3)
        #expect(received.isDuplicate == false)
        #expect(received.plaintext == Data("still here".utf8))
    }

    @Test("forgetPeer drops the session — sending fails afterwards")
    func sessionForgetPeer() throws {
        let alice = try Session()
        let bob = try Session()
        let bobId = try bob.identityPublic()
        try alice.initiateSession(peerIdentity: bobId, bundle: try bob.publishBundle())
        _ = try alice.encryptSealed(
            peer: bobId, messageId: Data(repeating: 0, count: 16),
            plaintext: Data("hi".utf8)
        )
        try alice.forgetPeer(peerIdentity: bobId)
        #expect(throws: (any Error).self) {
            _ = try alice.encryptSealed(
                peer: bobId, messageId: Data(repeating: 1, count: 16),
                plaintext: Data("again".utf8)
            )
        }
    }
